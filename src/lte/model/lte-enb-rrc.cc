...
UeManager::RecvRrcConnectionResumeRequestNb (NbIotRrcSap::RrcConnectionResumeRequestNb msg)
{
  ...
  switch (m_state)
    ...
    case CONNECTION_SETUP:
    case CONNECTION_RESUME:
      // NTN fix: resume request received in non-suspended state — ignore
      NS_LOG_WARN ("NTN: Resume request in state " << ToString (m_state) << " — ignored");
      break;

    default:
      NS_LOG_WARN ("NTN: Resume request in unexpected state " << ToString (m_state) << " — ignored");
      break;
    }
}
...
LteEnbRrc::LteEnbRrc ()
  : m_x2SapProvider (0),
    ...
    m_edt(true),
    m_haveSib31Ntn (false),
    m_haveSib32Ntn (false)
{
  NS_LOG_FUNCTION (this);
  m_legacy_lte = false;
...
LteEnbRrc::GetUeManagerbyRnti (uint16_t rnti)
{
  ...
  // NTN fix: UeManager may not exist for suspended NB-IoT UEs attempting resume
  if (it == m_ueActiveMap.end ())
  {
      NS_LOG_WARN ("UE manager for RNTI " << rnti << " not found — NB-IoT resume skipped");
      return 0; // return null — caller must check
  }
  return it->second;
}
...
{
  NS_LOG_FUNCTION (this << rnti);
  Ptr<UeManager> ueMan = GetUeManagerbyRnti (rnti);
  if (!ueMan) { NS_LOG_WARN ("RRC ConnReq: RNTI " << rnti << " not found"); return; }
  ueMan->RecvRrcConnectionRequest (msg);
}
...
{
  NS_LOG_FUNCTION (this << rnti);
  // NTN fix: guard against null UeManager for suspended UEs
  Ptr<UeManager> ueMan = GetUeManagerbyRnti (rnti);
  if (!ueMan)
  {
      NS_LOG_WARN ("NTN: RRC Resume for RNTI " << rnti << " skipped — UeManager not found");
      return;
  }
  if(DoCheckIfResumeIdExists(msg.resumeIdentity) && m_admitRrcConnectionResumeRequest){
    ResumeUe(rnti, msg.resumeIdentity);
  }
  ueMan->RecvRrcConnectionResumeRequestNb (msg);
}
...
LteEnbRrc::SendSystemInformationNb ()
{
  ...
  // NTN Rel-17: Include SIB31-NB if constellation manager provides it
  // SIB31 values (ta_common, k_offset, ephemeris) are set externally
  // by the scenario via SetNtnSib31() or NtnConstellationManager
  si.haveSib31 = m_haveSib31Ntn;
  if (m_haveSib31Ntn)
    si.sib31 = m_sib31Ntn;
  si.haveSib32 = m_haveSib32Ntn;
  if (m_haveSib32Ntn)
    si.sib32 = m_sib32Ntn;
