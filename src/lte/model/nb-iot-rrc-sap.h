...
class NbIotRrcSap{
    public:
        ...
        // 3GPP Rel-17 NTN: SIB31-NB (TS 36.331 Sec 6.7.4)
        struct SystemInformationBlockType31Nb
        {
            int32_t  taCommon;
            int16_t  taCommonDrift;
            uint16_t taCommonDriftVariation;
            uint16_t kOffset;
            uint16_t epochTimeSfn;
            uint8_t  epochTimeSubframe;
            struct StateVector {
                int32_t positionX, positionY, positionZ;
                int16_t velocityX, velocityY, velocityZ;
            } stateVector;
            struct OrbitalParams {
                uint32_t semiMajorAxis;
                uint16_t eccentricity, periapsis, longitude, inclination, meanAnomaly;
            } orbitalParams;
            bool useStateVector;
            uint16_t ulSyncValidityDuration;
            uint8_t ntnPolarizationDL;
            uint8_t ntnPolarizationUL;
            SystemInformationBlockType31Nb ()
                : taCommon(0), taCommonDrift(0), taCommonDriftVariation(0),
                  kOffset(20), epochTimeSfn(0), epochTimeSubframe(0),
                  useStateVector(true), ulSyncValidityDuration(60),
                  ntnPolarizationDL(0), ntnPolarizationUL(0)
            { stateVector = {0,0,0,0,0,0}; orbitalParams = {0,0,0,0,0,0}; }
        };

        // 3GPP Rel-17 NTN: SIB32-NB (TS 36.331 Sec 6.7.4)
        struct SystemInformationBlockType32Nb
        {
            struct SatelliteInfo {
                SystemInformationBlockType31Nb::StateVector ephemeris;
                uint16_t cellId;
                bool valid;
                SatelliteInfo () : cellId(0), valid(false) { ephemeris = {0,0,0,0,0,0}; }
            };
            uint8_t numSatellites;
            SatelliteInfo satellites[4];
            SystemInformationBlockType32Nb () : numSatellites(0) {}
        };
        ...
        struct SystemInformationNb
        {
            ...
            // NTN Rel-17 additions (TS 36.331 Sec 6.7.4)
            bool haveSib31;
            SystemInformationBlockType31Nb sib31; ///< SIB31-NB (NTN)
            bool haveSib32;
            SystemInformationBlockType32Nb sib32; ///< SIB32-NB (NTN)
            SystemInformationNb () : haveSib2(false), haveSib23(false), haveSib31(false), haveSib32(false) {}
        };
