...
LteUeMac::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::LteUeMac")
          ...
          .AddTraceSource ("RachAttempt", "trace fired on each RACH preamble attempt (NTN IDS)",
                           MakeTraceSourceAccessor (&LteUeMac::m_rachAttemptTrace),
                           "ns3::TracedCallback::Uint8Callback")
...
LteUeMac::RandomlySelectAndSendRaPreambleNb ()
{
  ...
  // NTN: fire RACH attempt trace for IDS feature extraction
  m_rachAttemptTrace ((uint8_t)m_CeLevel.coverageEnhancementLevel);

  // NTN-MISS-7: TA pre-compensation in Msg1 (TR 36.763 Sec 6.2)
  // UE applies timing offset based on m_ntnTaPrecompensation (set from SIB31 ephemeris + GNSS)
  // This shifts the NPRACH preamble timing to compensate for satellite propagation delay
  // m_ntnTaPrecompensation is in Ts units, set by scenario from SIB31.taCommon + UE-specific offset
  NS_LOG_INFO ("NTN RACH: TA pre-comp=" << m_ntnTaPrecompensation << " Ts, NTN mode=" << m_ntnMode);
