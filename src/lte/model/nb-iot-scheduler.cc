      ...
      if (test.size () > 0) // WE GOT A DOWNLINK NPDCCH CANDIDATE
        {
          ...
          std::vector<uint64_t> npdschsubframes = GetNextAvailableNpdschCandidate (
              *(test.end () - 1), m_minSchedulingDelayDci2Downlink + m_ntnKOffset, subframesNpdsch, ssc.R_max);
              ...
              if (message.isRar)
              ...
              else
                {
                  ...
                  std::vector<std::pair<uint64_t, std::vector<uint64_t>>> npuschharqsubframes =
                      GetNextAvailableNpuschCandidate (*(npdschsubframes.end () - 1), m_ntnKOffset,
      ...
      if (test.size () > 0)
        {
          ...
          std::vector<std::pair<uint64_t, std::vector<uint64_t>>> npuschsubframes =
              GetNextAvailableNpuschCandidate (*(test.end () - 1), m_ntnKOffset, subframesNpusch, true);
