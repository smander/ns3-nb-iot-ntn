...
private:
  ...
  /**
   * Trace fired on each RACH preamble attempt.
   * Parameter: CE level (0/1/2) of the attempt.
   * Added for NTN IDS feature extraction (window_conn_attempts).
   */
  TracedCallback<uint8_t> m_rachAttemptTrace;

  // NTN: TA pre-compensation value (set by scenario from SIB31 + GNSS)
  double m_ntnTaPrecompensation; ///< TA pre-compensation in Ts units
  bool m_ntnMode; ///< true if NTN mode enabled (skip TAT expiry)
};
